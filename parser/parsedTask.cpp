...
string Fact::toStringShort(const vector<Function>& functions, const vector<Object>& objects) {
    string s = "(" + functions[function].name;
    for (unsigned int i = 0; i < parameters.size(); i++)
        s += " " + objects[parameters[i]].name;
    return s + ")";
}
...
bool ParsedTask::compatibleTypes(Type* t, Variable* v)
{
    for (unsigned int vtype : v->types)
        if (vtype == t->index) return true;
    return compatibleTypes(t->parentTypes, v->types);
}
